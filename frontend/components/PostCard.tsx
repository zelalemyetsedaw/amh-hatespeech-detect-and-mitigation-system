...
const PostCard = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const response = await axios.get<Post[]>(
        "http://localhost:5012/api/Post/getwithouthate"
      );
      const postsWithUsername = await Promise.all(
        response.data.map(async (post) => {
          ...
          return { ...post, username, commentsCount };
        })
      );
      setPosts(postsWithUsername.sort((a, b) => dayjs(b.createdAt).diff(dayjs(a.createdAt))));
    };
  ...
  const handleReport = async (post: Post) => {
    ...
    try {
      ...
      setMenuVisible((prev) => ({ ...prev, [post.id]: false }));
      setReportReason("");
      setReportSuccess("Report sent successfully!"); // Set success message
      setTimeout(() => setReportSuccess(null), 10000); // Clear success message after 3 seconds
      console.log("success");
    } catch (error) {
  ...
  const fetchCommentsCount = async (postId: string) => {
    try {
      const response = await axios.get(`http://localhost:5012/api/Comment/bypost/${postId}`);
      return response.data.length; // Assuming the API returns an array of comments
    } catch (error) {
  ...
  return (
    ...
    {reportSuccess && (
      ...
      {posts.map((post) => (
        ...
        <div key={post.id} className="border-2 mb-3 bg-white">
          ...
          <div className="flex gap-2 p-2 pl-10">
            <IoPerson className="w-10 h-10 rounded-lg bg-custom1" />
            <div>
          ...
          {selectedPostId === post.id && <Comments commentsUrl="http://localhost:5012/api/Comment"
